<?php
session_start();
?>

<!DOCTYPE html>
<html lang="NL">
    <head>
        <link rel="stylesheet" href="Styles.css">
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Nora's Flora</title>
    </head>
   
    <body class="body-Home-Page">

        <header>
            <img src = "Asset/Nora'sFloraLogo.png" alt ="Logo" width="60px" height="40px" class="Logo">
            <div class="header-actions">
                <?php if (isset($_SESSION['user_id'])): ?>
                    <a href="logout.php" class="user-profile-button" aria-label="Logout">
                        <img src="Asset/1244531_v9_aa.jpg" alt="Logout" class="user-profile-icon">
                    </a>
                <?php else: ?>
                    <a href="LoginPage.html#login" class="user-profile-button" aria-label="Login/Register">
                        <img src="Asset/1244531_v9_aa.jpg" alt="Login/Register" class="user-profile-icon">
                    </a>
                <?php endif; ?>
                <a href="ShoppingCart.html">
                    <img src = "Asset/ShoppingCart.png" alt ="Shopping Cart" width="40px" height="40px" class="shopping-cart">
                </a>
            </div>
        </header> 

        <nav>   
            <a href="index.html">Home page</a>
            <a href="Assortiment.html">Assortiment</a>
            <a href="Contact.html">Contact</a>
        </nav>


    <section class="welcome-message">
        <img src="Asset/cat-sticker-1.png" alt="" class="cat-sticker cat-sticker-1">
        <img src="Asset/cat-sticker-2.png" alt="" class="cat-sticker cat-sticker-2">
        <img src="Asset/cat-sticker-3.png" alt="" class="cat-sticker cat-sticker-3">
        <img src="Asset/cat-sticker-4.png" alt="" class="cat-sticker cat-sticker-4">
        <img src="Asset/cat-sticker-5.png" alt="" class="cat-sticker cat-sticker-5">
        <img src="Asset/cat-sticker-6.png" alt="" class="cat-sticker cat-sticker-6">
        <img src="Asset/cat-sticker-7.png" alt="" class="cat-sticker cat-sticker-7">
        <img src="Asset/cat-sticker-8.png" alt="" class="cat-sticker cat-sticker-8">
        <img src="Asset/cat-sticker-9.png" alt="" class="cat-sticker cat-sticker-9">
        <h1>Welkom bij Nora's Flora!</h1>
        <p>De beste persoonlijke bouqetten en bloem in Zwolle</p>
    </section>

        <h5 class="bloemetjes">Bloemetjes die in het zonnetje gezet worden</h5>

    <figure class="figure-index"> 
        <div class="flower-index">
            <img src="Asset/Sunflower.webp" alt="Zonnebloemen">
            <figcaption>Zonnebloemen<br>€ 2,- per bos deze maandag</figcaption>
        </div>

        <div class="flower-index">
            <img src="Asset/Tulips.jpg" alt="Rode Tulpen">
            <figcaption>Rode Tulpen<br>€ 3,- per bos deze dinsdag</figcaption>
        </div>

        <div class="flower-index">
            <img src="Asset/Cactus.jpg" alt="Cactus in pot">
            <figcaption>Cactus in pot<br>Deze week, 3 voor €5,-</figcaption>
        </div>

        <div class="flower-index">
            <img src="Asset/pexels-nextvoyage-457937.jpg" alt="Speciaal">
            <figcaption>Speciaal deze maand<br>Import gras uit Toronto Canada</figcaption>
        </div>

        <div class="flower-index">
            <img src="Asset/purple flower.jpg" alt="Paarse Allium">
            <figcaption>Paarse Allium Bloemen<br>Gratis als extra bij een bouquet</figcaption>
        </div>

        <div class="flower-index">
            <img src="Asset/Diversen.jpg" alt="Vetbloemen">
            <figcaption>Vetbloemen deze week<br>3 + 1 gratis</figcaption>
        </div>
    </figure>

    <article>
    <section class="content">
        <div class="about-us">
        <div class="text-content">
            <h2>Welkom bij Nora's Flora!</h2>
            <p> 
            Bij Nora's Flora draait alles om de liefde voor bloemen 
                <br>
            en het creëren van iets moois voor ieder moment.
                <br>
                <br>
            Ik ben Nora, de trotse eigenaresse, en ik stel met veel zorg en 
                <br>
            passie elk boeket samen met oog voor kleur, vorm en seizoen.
                <br>
                <br>
            Of je nu iemand wilt verassen, een bruiloft plant of gewoon je dag wilt 
                <br>
            opfleuren, bij Nora's Flora vind je altijd iets bijzonders. Elk boeket wordt 
                <br>
            met de hand gemaakt en afgestemd op jouw wensen, met bloemen van 
                <br>
            lokale en duurzame kwekers.
                <br>
                <br>
            Met bloemen van Nora's Flora wordt elke dag een beetje vrolijker!
            </p>
        </div>
        </div>
            <img src="Asset/pexels-olly-3775156.jpg" alt="Nora's Flora" class="info-image">
        </section>
    </article>

<footer class="footer footer-index">
    <div class="footer-container">
    <div class="footer-left">
        <p>
        <strong>Email</strong>              :contact@noraflora.com<br>
        <strong>Telefoon</strong>           :06 12345678<br>
        <strong>Adres</strong>              :Zwolle, pannenkoekendijk 420 B</p>
    </div>
    <div class="footer-right">
      <h3>Openingstijden</h3>
      <p>
        Ma - Vr : 12:00 - 17:00<br>
        Zaterdag : 10:00 - 17:00
      </p>
    </div>
  </div>
</footer>
</body>
</html>